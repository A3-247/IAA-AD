...
# ex_RMSE = pymp.shared.array(1024, dtype='float')
# ex_MSE =  pymp.shared.array(1024, dtype='float')
# ex_MAE =  pymp.shared.array(1024, dtype='float')
# ex_R2  =  pymp.shared.array(1024, dtype='float')
...
def regression(epoch, num):
    ...
    with pymp.Parallel(4) as P:
        for index in P.range(0, 60, 1):
            ...
            print("epoch:", iii, f'MSE: {MSEEE[num - 1][iii]}', f'RMSE: {RMSE[num - 1][iii]}',
                  f'MAE: {MAEEE[num - 1][iii]}', f'R^2: {R222[num - 1][iii]}')
...
def lossR(epoch, YR, op):
    ...
    with open('./R.txt', 'a') as f:
        f.write("epoch:" + str(epoch) + " YR:" + str(YR) + " op:" + str(op) + " rmse:" + str(rmse) + " mae:" + str(
            mae) + " r2:" + str(r2) + "\n")
        f.close()
    if op == 1:
